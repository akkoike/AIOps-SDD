...
注記:
- `docs/` 配下は「暗黙知を仕様へ変換する特別ケース」のために準備した領域です。
- この AI 駆動開発の通常フローでは、`docs/` 配下は基本的に利用せず、`categories/` 配下を直接更新します。
...
- `docs/` は暗黙知の仕様化用として保持しており、通常の AI 駆動開発では利用しない運用を原則とします
