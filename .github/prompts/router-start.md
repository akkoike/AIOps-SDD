...
2. `.github/agents/sdd-hearing-subagent-sample.md` に従い、8 つの質問を順番に実施する（各質問で選択肢 + 自由入力）。
   ...
   - **Azure との通信が必要な依頼（Azure 連携＝「必要」）の場合は**、アカウント名・サブスクリプション名・サブスクリプションID・対象リソースグループ名を追加でヒアリングし、**Azure へ通信する工程を開始する前に `Account/accounts.yml` へセット**する。
...
[STEP 1] ヒアリング
  ...
  （Azure 連携が必要な場合は Azure 接続情報も収集し accounts.yml へセット）
      ↓
[STEP 2] カテゴリ判定
...
3. **Azure連携要否を確認し、「必要」の場合は Azure 接続情報（アカウント名・サブスクリプション名・サブスクリプションID・対象リソースグループ名）を追加収集する**
4. 回答不足がある場合は、同サブエージェントで追加質問を実施する
5. 収集結果を下記のヒアリング結果テンプレートに転記する
...
- Azure接続情報（Azure連携＝「必要」の場合のみ）:
  - アカウント名: {{UPN / account@domain}}
  - サブスクリプション名: {{サブスクリプション表示名}}
  - サブスクリプションID: {{GUID形式のサブスクリプションID}}
  - 対象リソースグループ: {{リソースグループ名（複数可） / RG指定なし}}
  - accounts.yml 反映状況: {{反映済み / 未反映}}
...
12. **Azure連携＝「必要」の場合は、Azure へ通信する工程を開始する前に、ヒアリングで収集した接続情報を `Account/accounts.yml`（`account_name` / `subscription_name` / `subscription_id` / `resource_groups[]`）へセットする。セット完了を確認してから Azure 通信を伴う処理を実行する。未設定のまま Azure へ通信してはならない。**
...
- ガード7: Azure連携＝「必要」の場合、Azure へ通信する処理（リソース参照・作成・変更・コスト/権限取得など）を実行する前に、ヒアリングで収集した接続情報を `Account/accounts.yml` へセット済みであることを確認する。未設定（`${...}` プレースホルダのまま）の場合は Azure 通信を実行せず、不足情報を追加ヒアリングで補完してから続行する。
